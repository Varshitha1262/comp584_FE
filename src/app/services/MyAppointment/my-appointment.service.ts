...
export class MyAppointmentService {
  ...
  appointments(data: BookAppointment): Observable<any> {
    let API_URL = `${this.REST_API}/my-appointments`;

    this.currentData = this.token.getUser();
    this.currentData = this.currentData.data;

    const uuser_id = this.currentData.user.patient.patient_id;

    data.patient_id = uuser_id;
  ...
  update_status(data: BookAppointment, pass_id:number, status:String): Observable<any> {
    ...
    // const uuser_id = this.currentData.id;
    // const id = pass_id;
